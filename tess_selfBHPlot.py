...
f1=open('./files_Figs/B{0:d}_1.dat'.format(sector) )
N0=int(len(f1.readlines()))
################################################################################

for ll in range(1): 
    f1=open("./files_Figs/B{0:d}_1.dat".format(sector),"r")
    nf= sum(1 for line in f1)  
    par=np.zeros((nf,nc))
    par=np.loadtxt("./files_Figs/B{0:d}_1.dat".format(sector)) 
    for i in range(nf):
        ...
        depth,depthO, depthL, rho=par[i,29],par[i,30],par[i,31],par[i,32]
        logg,impactL, impactO, RE= par[i,33],par[i,34],par[i,35],par[i,36]
        ...
        if(FlagD>0): 
            nd=-1;  nm=-1;  
            try: 
                f1=open('./files_Figs/L_{0:d}_1.dat'.format(nsim) )
                nd=int(len(f1.readlines()))
                print(nd)
            except: 
                print("file does not exist",  nsim)    
            try:
                f2=open('./files_Figs/M_{0:d}_1.dat'.format(nsim) )
            ...
            if(nd>1 and nm>1): 
                dat=np.zeros((nd,3))
                dat=np.loadtxt('./files_Figs/L_{0:d}_1.dat'.format(nsim))
                for ii in range(nd): 
                    dat[ii,1]=dat[ii,1]+np.random.normal(0.0,dat[ii,2],1)*0.3
                
                 
                mod=np.zeros((nm,13))
                mod=np.loadtxt('./files_Figs/M_{0:d}_1.dat'.format(nsim)) 
                ...
                plt.title(
                r"$M_{\rm{BH}}(M_{\odot})=$"+'{0:.1f}'.format(Ml)+
                r"$,~T(\rm{days})=$"+'{0:.2f}'.format(period)+
                r"$,~\log_{10}[a/R_{\star}]=$"+'{0:.2f}'.format(lsemi)+
                r"$,~\rho_{\star}=$"+'{0:.2f}'.format(rho)+ "\n"+ 
                ...
                fig.savefig("./Figs/lightB/LightB{0:d}.jpg".format(nsim),dpi=200)
                ################################################################
                '''
