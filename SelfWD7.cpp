...
///HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
   ...
   double ksi, x0, y0, x1, y1, z1, dis, yc, zc, dec, ra, xp, yp, vx; 
...
///=============================================================================
   ...
   sprintf(filenam0,"./files/light/lcF2/%c%d%c%d.dat",'D',sector,'_',7);
...
///=============================================================================
  ...
  // srand(time(0)); 
   ...
   if(!magdi){cout<<"cannot read CTLc.dat "<<sector<<endl; exit(0);}
   for(int i=0; i<num; ++i){
   ...
   if(nsim<0){
   ffd=1;
   sprintf(filenam1,"./files/light/lcF2/%c%c%d%c%d.dat",'L','_',int(nsim),'_',0);
   fild=fopen(filenam1,"w");
   sprintf(filenam2,"./files/light/lcF2/%c%c%d%c%d.dat",'M','_',int(nsim),'_',0);
   ...
   FlagD=1; 
   Amin=1000.0;  Amax=0.0;  error=0.0; xp=-2.00; x1=0.0;  yp=-2.00;  signD=0;
   ...
   "%.6lf  %.2lf  %.6lf  %.6lf  %.5lf  %.5lf  "//23
   "%.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %d "  //30
   "%.5lf  %.5lf  %.6lf  %.7lf  %.8lf  %.6lf  %.8lf "//37
   "%.6lf  %.5lf  %.5lf  %e  %e   %e   %e  %.8lf  %.6lf %.8lf  %.8lf  %e\n", //49
   FlagD, nsim, signD, s.lat, s.lon, //5   
   s.Ds, s.mass, s.Tstar, s.Rstar, s.limb, s.Mab, s.Map, s.magb, s.blend, s.nsbl, s.Ai, s.prior,//17
   l.MBH, l.teff[l.num], l.RBH/Rsun, l.logg[l.num], l.Map, l.Mab,//23
   l.magG, l.magBP, l.magRP, l.G[l.num], l.BP[l.num], l.RP[l.num],l.num, //30
   l.inc*RA, l.tet*RA, l.ecen, l.period,  log10(l.a/(s.Rstar*Rsun)), l.tp, l.ratio,//37
   error, d.dchi, snr,s.cdpp, depth,double(1.0-Amin),double(Amax-1.0),Rho,s.loggs,impactL,impactO,RE);//49
   fclose(distr);      
   cout<<"=============================================================="<<endl;
...
{    
  ...
  /*
  double cosi=RandR(cos(20.0*M_PI/180.0),cos(0.0)); 
  l.inc=acos(cosi);
  if(l.inc<-0.001 or l.inc>double(20.001*M_PI/180.0)){
  ...
  double Dist=double(l.dist[l.num]);  
  if(ex.flag>0) Av=double(Interpol(Dist,ex)*Avks);
  else          Av=double(0.7*Dist); 
  if(Av<0.0)    Av=0.0;
