...
///=============================================================================
   ...
   sprintf(filenam0,"./files/light/lcF2/%c%d%c%d.dat",'B',sector,'_',11);
   distr=fopen(filenam0,"a+"); 
   fclose(distr);              
      
///=============================================================================
   
   for(nsim=1;  nsim<=10000; ++nsim){
  ...
  // srand(time(0)); 
   ...
   sprintf(filenam7,"./files/%c%c%c%c.dat",'C','T','L','c');
   magdi=fopen(filenam7,"r"); 
   if(!magdi){cout<<"cannot read CTLb.dat "<<sector<<endl; exit(0);}
   for(int i=0; i<num; ++i){
   fscanf(magdi,"%d  %lf  %lf  %lf  %lf  %lf  %lf  %lf  %lf  %lf  %lf  %lf %lf  %lf %lf\n",
   &ID, &ra, &dec, &t.glong, &t.glat, &t.elong, &t.elat, &t.Map, &t.error, &t.teff, &t.radius, &t.dist, &t.blend, &t.prior,&t.mass);
   ...
   if(nsim<0){
   ffd=1;
   sprintf(filenam1,"./files/light/lcF2/%c%c%d%c%d.dat",'L','_',int(nsim),'_',1);
   fild=fopen(filenam1,"w");
   sprintf(filenam2,"./files/light/lcF2/%c%c%d%c%d.dat",'M','_',int(nsim),'_',1);
   ...
   FlagD=1; 
   Amin=1000000.0;  Amax=0.0;  error=0.0; xp=-2.0;   x1=0.0;     yp=-2.0;  
   d.numt=0;     chi1=0.0;  chi2=0.0;  timee=0.0;    signD=0;
   impactL=100000000.0; impactO=10000000000.0;   Rho=0.0;  RE=0.0; 
   ...
   /* 
   Occul=1.0; 
   if(x1>0.0 and l.MBH<2.17){//Occultation  
   if(us<=0.0){
   ...
   d.dchi=0.0;  error=fabs(ErrorTESS(s.magb));  depth=fabs(1.0-1.0/(1.0+l.ratio));   FlagD=0;   impactL=-10.0; 
   ...
   "%.6lf  %.6lf  %.5lf  %.5lf  %.6lf  %.7lf  %.8lf  %.6lf "//25
   "%.6lf  %.5lf  %.8lf  %.8lf  %e     %e   %e   %.8lf  %.3lf  %.8lf  %.8lf  %.9lf\n", //37
   FlagD, nsim, signD, s.lat, s.lon, //5   
   s.Ds, s.mass, s.Tstar, s.Rstar, s.limb, s.Mab, s.Map, s.magb, s.blend, s.nsbl, s.Ai, s.prior,//17
   l.MBH, l.RBH/Rsun, l.inc*RA, l.tet*RA, l.ecen, l.period, log10(l.a/(s.Rstar*Rsun)),l.tp,//25
   error, d.dchi,snr,s.cdpp,depth,double(1.0-Amin),double(Amax-1.0),Rho,s.loggs,impactL,impactO,RE);//37
   fclose(distr);      
   cout<<"=============================================================="<<endl;
...
{    
  ...
  l.tet=RandR(0.01,359.99)*M_PI/180.0;//rad
  ...
  l.MBH=RandR(3.3, 50.0);//in solar mass BH
  ...
  l.RBH=2.0*G*l.MBH*Msun/velocity/velocity ;//l.radius[l.num]*Rsun;//meter 
